...
- **Two tabs, switched bottom-left** (`activeTab` in `App.tsx`): **Capacity Matrix** (default,
  read-only `MatrixView.tsx` — people × objectives, collapsible Initiative column-groups,
  Project filter, one row per person·role, `Per role` + `Per person` totals, per-objective /
  per-initiative footers, ticket links) and the original **What-if Dashboard**.
- Matrix grouping reads `project`/`initiative`/`team` from `loe_allocations.csv`; `data.ts`
  falls back to **"Unspecified"** for CSVs lacking those columns (live branch until the Action
  re-runs). Don't drop that fallback.
